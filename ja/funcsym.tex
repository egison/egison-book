...
引数を代入した結果も\lstinline{MathExpr}型の数式であるため，さらに微分できる．
...
\lstinline{Function}データコンストラクタは二つの引数をとる．
第一引数には関数シンボルの名前（\lstinline{ScalarData}型）がはいる．
第二引数には関数シンボルの引数の値のリスト（\lstinline{[ScalarData]}型）がはいる．
...
連鎖律の実装では，引数の数を利用して各引数に対する偏微分係数を整数インデックスで取り出している．
...
\lstinline{userRefs f [1]}は\lstinline{f}の名前シンボルに整数インデックス\lstinline{1}を付加した数式\lstinline{f|1}を返す．
