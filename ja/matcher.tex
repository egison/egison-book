...
引数マッチャーの位置には\emph{スロット型}\lstinline{MatcherSlot a b}が使われる．
第一成分\lstinline{a}はその位置のターゲットの型，第二成分\lstinline{b}はその位置のパターンが要求する分解の型を表すが，
\lstinline{list}や\lstinline{multiset}のように両者が一致する場合は「おおよそ\lstinline{Matcher a}を受け取る」と読んでよい．
...
matchAll [1, 2, 3] as multiset integer with
  | $x :: #(x ++ [1]) :: _ -> x
-- `型エラー: (++)は[Integer]を期待するがxはInteger`
\end{lstlisting}
つぎに，値パターンを置く位置の誤りも検出される．
\begin{lstlisting}[language=egison,numbers=none]
matchAll [1, 2, 3] as multiset integer with
  | $x :: #x -> x
-- `型エラー: ::の第二引数の位置はPattern [Integer]だが`
-- #x`はPattern Integer`
\end{lstlisting}
...
matchAll [1, 2, 3] as multiset integer with
  | #x :: $x :: _ -> x
-- `警告: 未束縛変数`x`（束縛`$x`より左で`#x`を参照している）`
\end{lstlisting}
...
matchAll 5 as integer with
  | $x :: _ -> x
-- `型エラー: ::はコレクション[a]を対象とするが`
-- `ターゲットとマッチャーの対象はInteger`
\end{lstlisting}
ボディの型の不一致も検出される．
...
matchAll [1, 2] as multiset integer with
  | twin ($y :: _) _ -> y
-- `型エラー: 第一引数はPattern Integerを期待するが`
-- $y :: _`はPattern [Integer]`
\end{lstlisting}
