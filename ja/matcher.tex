...
\section{マッチャーとパターンの型}\label{matcher-types}

ここまでのマッチャー定義には，\lstinline{Matcher (Integer, Integer)}のような型注釈が現れていた．
本節からは，パターンマッチが型システムでどのように検査されるかを解説する．

Egisonの型システムでは，マッチャーとパターンにそれぞれ型が付く．
\lstinline{Matcher a}は\lstinline{a}型の値を対象とするマッチャーの型であり，\lstinline{Pattern a}は\lstinline{a}型の値にマッチするパターンの型である．
マッチャーは第一級の値なので，\lstinline{multiset}のようにマッチャーを引数にとってマッチャーを返す関数も型をもつ．
たとえば，コレクションの基本マッチャーは以下の型をもつ．
\begin{lstlisting}[language=egison,numbers=none]
something : Matcher a
eq        : {Eq a} => Matcher a
...
\end{lstlisting}
引数マッチャーの位置には，\lstinline{Matcher}ではなく\emph{スロット型}\lstinline{MatcherSlot a b}が使われる．
スロット型は2つの型をとる．
第一成分\lstinline{a}はその位置で使われるパターンが要求する分解（構造）の型，
第二成分\lstinline{b}はその位置のターゲット（分解される値）の型である．
\lstinline{list}や\lstinline{multiset}の要素位置のように両者が一致する場合は，「おおよそ\lstinline{Matcher a}を受け取る」と読んでよい．
スロット型が分解の型とターゲットの型を別々にもつのは，\textbf{そこに渡されるマッチャーがその位置のパターンを実際に分解できるか}を，
渡された場所ごとに検査するためである．この検査については\ref{matcher-match-typing}節で改めて扱う．

この型の一覧には設計上の要点がひとつ隠れている．
\lstinline{list}や\lstinline{multiset}の型に\lstinline{Eq}制約が\emph{付いていない}ことである．
値パターン（\lstinline{#5}など）の照合は\lstinline{something}のレベルで構造的な等価性により処理されるため，
コレクションのマッチャー自体は要素の等価性を要求しない．
そのおかげで，等価性が定義されていない型（たとえばシンボリックな数式や抽象構文木）に対しても
\lstinline{multiset}などのマッチャーをそのまま使うことができる．
第\ref{quick}章\ref{quick-matcher-composition}節の\lstinline{intersect}のように\lstinline{eq}マッチャーを明示的に使うのは，
\lstinline{Eq}型クラスのインスタンスによるカスタムの等価性で照合したい場合である．

\section{パターンの型検査}\label{matcher-pattern-typing}

パターンには，どのマッチャーで使われるかと独立に型が付く（\emph{マッチャー多相}）．
たとえば\lstinline{$x :: $xs}は\lstinline{Pattern [a]}という型をもち，
だからこそ同じパターンを\lstinline{list integer}でも\lstinline{multiset integer}でも使うことができる．

値パターンの型検査には束縛の順序が関わる．
パターンは左から右へ向かって束縛を増やしながら検査され，
値パターン\lstinline{#e}の式\lstinline{e}は，それまでに束縛されたパターン変数を参照できる．
\begin{lstlisting}[language=egison,numbers=none]
matchAll [1, 2, 3, 1] as multiset integer with
  | $x :: #x :: _ -> x
-- [1, 1]
\end{lstlisting}
このパターンは「ある要素\lstinline{x}と，それと同じ値の要素がもうひとつある」ことを表す非線形パターンであり，
\lstinline{#x}は直前の\lstinline{$x}の束縛（\lstinline{Integer}型）の下で型検査される．

型検査は，動的な処理系では黙って失敗するだけだった誤りを静的に検出する．
まず，値パターンの中の式の型の誤りは，そのまま型エラーとして報告される．
\begin{lstlisting}[language=egison,numbers=none]
matchAll [1, 2, 3] as multiset integer with
  | $x :: #(x ++ [1]) :: _ -> x
...
\end{lstlisting}
プログラマの意図は「\lstinline{x}が残りにも現れる」だったはずだが，\lstinline{::}の第二引数はリスト全体にマッチする位置である．
正しくは\lstinline{$x :: #x :: _}と書く．
動的な処理系では，このパターンは何も報告せずにマッチに失敗するだけであった．
最後に，束縛より左で変数を参照する誤りは，未束縛変数の警告として検出される
（strictモードでは型エラーになる）．
\begin{lstlisting}[language=egison,numbers=none]
matchAll [1, 2, 3] as multiset integer with
  | #x :: $x :: _ -> x
...
\section{\texttt{match}式の型検査}\label{matcher-match-typing}

\lstinline{match}式・\lstinline{matchAll}式では，ターゲット・マッチャー・パターン・ボディの型がすべて整合しなければならない．
ターゲットの型が\lstinline{a}なら，マッチャーは\lstinline{Matcher a}，各マッチ節のパターンは\lstinline{Pattern a}であり，
すべてのマッチ節のボディの型は一致する必要がある．
...
\end{lstlisting}
ボディの型の不一致も検出される．
たとえば第\ref{quick}章\ref{quick-pat-fun}節の\lstinline{twin}の例で
ボディを\lstinline{[n, ns]}と書くと，\lstinline{Integer}と\lstinline{[Integer]}が混在するリストになるため型エラーになる
（タプル\lstinline{(n, ns)}を使う）．

さらに，パターンが要求する\emph{分解}をマッチャーが提供できるかも検査される（適合性検査）．
変数パターンと値パターンしか使わないパターンは分解を要求しないので，任意の型で\lstinline{something}と組み合わせられる．
一方，\lstinline{::}のようなパターンコンストラクタを使うパターンには，それを分解できるマッチャーが必要である．
\lstinline{something}は任意の値に単一のマッチをするだけで分解の手段をもたないため，
コンスパターンと組み合わせると型エラーになる．
\begin{lstlisting}[language=egison,numbers=none]
...
\end{lstlisting}
これが，マッチャーを受け取る位置を\lstinline{Matcher}ではなくスロット型で型付けする理由である．
ここでは，コンスパターンがスロットの分解成分に\lstinline{[_]}（リストの分解）を要求するのに対し，
\lstinline{something}はそれを提供できない．
素朴に\lstinline{something}を\lstinline{Matcher [Integer]}とみなす型システムでは，この式は型検査を通ってしまい，
実行時に\lstinline{something}がコンスを分解できずに行き詰まる．
Egisonの型システムは，マッチャーが渡された場所ごとに分解の適合を検査するので，この誤りを静的に弾く．
この検査はパターンの構造だけから計算されるため，マッチャーを実行しなくても誤った組み合わせが静的に弾かれる．

\section{マッチャー定義の整合性検査}\label{matcher-consistency}

マッチャー定義そのものも型検査の対象である．
各マッチャー節について，原始パターンパターン・ネクスト・マッチャー式・原始データパターンの型の整合が検査されるのに加えて，
\emph{原始データパターンのアームの網羅性}が要求される．
あるマッチャー節の原始パターンパターンがパターンにマッチすると，
ターゲットの照合はその節のアームだけで行われ，どのアームにもマッチしなければ実行時エラーになる．
そのため，アームの集合が網羅的でないマッチャー定義は型エラーとして拒否される．
\begin{lstlisting}[language=egison]
...
\end{lstlisting}
コンス・パターンの節のアームが\lstinline{$x :: $xs}だけだと，空リストがターゲットに来たときにどのアームにもマッチせず，
実行時に失敗してしまう．
末尾に\lstinline{| _ -> []}を追加すれば「分解できないときはマッチ失敗（候補なし）」という意味になり，網羅的になる．
網羅的と認められるアームの形は，（1）末尾のワイルドカード・アーム，（2）\lstinline{[]}と\lstinline{::}のペア（コレクションに対して完全），
（3）反駁不能な単一アーム（\lstinline{$tgt}など）である．

\section{パターン関数の型}\label{matcher-patfun-types}

第\ref{quick}章\ref{quick-pat-fun}節で導入したパターン関数にも型が付く．
\begin{lstlisting}[language=egison,numbers=none]
def pattern twin {a} (pat1: a) (pat2: [a]) : [a] :=
  (~pat1 & $x) :: #x :: ~pat2
\end{lstlisting}
この定義において，仮引数の型注釈\lstinline{(pat1: a)}は「\lstinline{pat1}は\lstinline{a}型の値にマッチするパターン」を表し，
返り値位置の\lstinline{: [a]}は「適用結果は\lstinline{[a]}型の値にマッチするパターン」を表す．
つまり\lstinline{twin}の型はおおよそ\lstinline{Pattern a -> Pattern [a] -> Pattern [a]}と読める．
適用時には引数パターンの型が検査される．
\begin{lstlisting}[language=egison,numbers=none]
...
パターン関数の適用は，適合性検査（\ref{matcher-match-typing}節）では\emph{本体のパターンの骨格}として扱われる．
\lstinline{twin}の本体はコンス・パターンの入れ子なので，\lstinline{twin}を使うマッチ式にはコンスを分解できるマッチャー
（\lstinline{list}や\lstinline{multiset}）が必要である．
逆に，本体が仮引数そのものであるようなパターン関数は構造を課さないため，引数パターンの要求だけが伝播する．
このように，パターン関数を経由しても「そのパターンにどんなマッチャーが必要か」が静的に追跡される．
