...
たとえば，$(p \lor q) \wedge (\neg p \lor r) \wedge (\neg p \lor \neg r)$は，$p = \texttt{False}$，$q = \texttt{True}$，$r = \texttt{True}$という割り当てをすれば，真になる連言標準形の論理式である．

\subsection{Davis-Putnamアルゴリズム}

Davis-Putnumアルゴリズム\cite{harrison2009handbook}というSAT問題を解くアルゴリズムのなかでもシンプルなアルゴリズムをここでは実装する．
本節の実装は，連言標準形の命題論理式をリテラルのコレクションのコレクションとして表現する．
$\wedge$と$\lor$は可換な演算子であるため，このコレクションのコレクションは，リテラルの多重集合の多重集合としてパターンマッチできる．
さらに，リテラルを整数として表現する．
整数の正負によって，リテラルが論理変数がその否定を表現する．
...
これにより，プログラムの期日しやすさ・読解しやすさが向上する．
