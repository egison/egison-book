...
Function symbols are treated as mathematical expressions of type \lstinline{MathExpr}.
...
The result of substituting arguments is also a mathematical expression of type \lstinline{MathExpr}, so it can be further differentiated.
...
def g : MathExpr := function (x, y)

show g
-- "g (r * 'cos θ) (r * 'sin θ)"

show (`$\partial$`/`$\partial$` g r)
-- "g|1 (r * 'cos θ) (r * 'sin θ) * 'cos θ + g|2 (r * 'cos θ) (r * 'sin θ) * 'sin θ"
\end{lstlisting}
...
Function symbols are internally represented in Egison using the \lstinline{Function} data constructor.
The \lstinline{Function} data constructor takes two arguments.
The first argument contains the name of the function symbol (of type \lstinline{ScalarData}).
The second argument contains the list of argument values of the function symbol (of type \lstinline{[ScalarData]}).
...
fromMathExpr f
-- Div (Plus [Term 1 [(Function f [x, y], 1)]]) (Plus [Term 1 []])
\end{lstlisting}

The \lstinline{func} pattern constructor can be used to pattern match on function symbols.
This pattern extracts the function name and the list of argument values.
...
In the chain rule implementation, the number of arguments is used to extract the partial derivative coefficient for each argument using integer indices.
...
def `$\partial$`/`$\partial$` $f *$x :=
  match f as mathExpr with
    ...
    | func _ $args ->
      sum (map2 (\s r -> (userRefs f [s]) * `$\partial$`/`$\partial$` r x)
...
\lstinline{userRefs f [1]} returns the expression \lstinline{f|1}, which is the name symbol of \lstinline{f} with the integer index \lstinline{1} appended.
...
These are managed as additional arguments to the \lstinline{eval} function in the Egison interpreter implementation.
