...
As another example, the \lstinline{Num} type class representing numeric types is defined as follows.
\begin{lstlisting}[language=egison,numbers=none]
class Num a where
  (+) (x: a) (y: a) : a
  (-) (x: a) (y: a) : a
...
An example of making the \lstinline{MathExpr} type (a type representing mathematical expressions) an instance of \lstinline{Num} is as follows.
\begin{lstlisting}[language=egison,numbers=none]
...
\end{lstlisting}
Here, \lstinline{dict_Num_a} is a hash (dictionary) containing the methods of the \lstinline{Num a} instance (\lstinline{+}, \lstinline{-}, \lstinline{*}, \lstinline{/}).
...
double numInteger 1
\end{lstlisting}
Here, \lstinline{numInteger} is the dictionary for the \lstinline{Num Integer} instance.
