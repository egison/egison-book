...
\end{lstlisting}
Argument-matcher positions use the \emph{slot type} \lstinline{MatcherSlot a b}, not \lstinline{Matcher}.
A slot type takes two types.
The first component \lstinline{a} is the type of the decomposition (structure) demanded by the pattern used at that position, and the second component \lstinline{b} is the type of the target (the value being decomposed) there.
When the two coincide, as at the element position of \lstinline{list} and \lstinline{multiset}, one may read it as ``takes roughly a \lstinline{Matcher a}''.
The reason a slot type carries the decomposition type and the target type separately is to check, \textbf{per use site, whether the matcher supplied there can actually decompose the pattern at that position}.
We return to this check in Section~\ref{matcher-match-typing}.
...
Since \lstinline{something} only matches any value as a whole and offers no means of decomposition, combining it with a cons pattern is a type error.
\begin{lstlisting}[language=egison,numbers=none]
...
\end{lstlisting}
This is why matcher-receiving positions are typed with a slot type rather than \lstinline{Matcher}.
Here the cons pattern demands \lstinline{[_]} (list decomposition) in the slot's decomposition component, which \lstinline{something} cannot provide.
A naive type system that treats \lstinline{something} as \lstinline{Matcher [Integer]} would accept this expression and then get stuck at runtime, where \lstinline{something} cannot decompose a cons.
Egison's type system checks decomposition admissibility per use site, so it rejects the error statically.
