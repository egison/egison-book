...
\section*{\lstinline{intersect}関数}
...
\section*{\lstinline{difference}関数}
...
\ref{practice}章\ref{practice-poker}節のポーカーの役判定のためのパターンマッチ（\lstinline{poker}関数）の実装を一切変更せずに，\lstinline{card}マッチャーの定義だけを変更して，ジョーカーの存在を考慮するポーカーの役判定をできるようにせよ．
