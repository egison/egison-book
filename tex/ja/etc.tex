...
Egisonは\lstinline{extends}キーワードによる\emph{スーパークラス宣言}もサポートしている．
型クラスは1つ以上のスーパークラスを宣言でき，サブクラスのインスタンスである型はスーパークラスのインスタンスでもなければならない．

これらの機能を使って，Egisonは数値型のための以下の代数的型クラス階層を定義している．
まず，加法の階層は以下のように定義される．
\begin{lstlisting}[language=egison,numbers=none]
...
class AddGroup a extends AddMonoid a where
  neg (x: a) : a
\end{lstlisting}
\lstinline{AddSemigroup}は加法を提供し，\lstinline{AddMonoid}はそれに零元を追加し，\lstinline{AddGroup}はさらに符号反転を追加する．
...
これらの階層を組み合わせて複合的な代数構造を定義する．
型クラスはカンマ区切りで\emph{複数}のスーパークラスを継承できる．
独自のメソッドをもたない型クラスは，スーパークラスを組み合わせるだけの\emph{マーカークラス}として機能する．
\begin{lstlisting}[language=egison,numbers=none]
class Ring a extends AddGroup a, MulMonoid a
class Field a extends Ring a, MulGroup a
\end{lstlisting}
\lstinline{Ring}は加法群と乗法モノイドの両方の構造を要求し，\lstinline{Field}はさらに乗法逆元を要求する．
...
\lstinline{MathValue}型を代数的型クラス階層のインスタンスにする例は以下のとおりである．
階層の各レベルごとにインスタンス定義が必要になる．
\begin{lstlisting}[language=egison,numbers=none]
...
メソッドをもたないマーカークラスのインスタンスは\lstinline{where}句なしで宣言できる．
\begin{lstlisting}[language=egison,numbers=none]
...
\end{lstlisting}
同様に，\lstinline{sum}と\lstinline{product}も適切な制約で定義される．
\begin{lstlisting}[language=egison,numbers=none]
...
\end{lstlisting}
ここで\lstinline{dict_AddSemigroup_a}は\lstinline{AddSemigroup a}インスタンスのメソッド（この場合は\lstinline{+}）を含むハッシュ（辞書）である．
...
\end{lstlisting}
ここで\lstinline{addSemigroupInteger}は\lstinline{AddSemigroup Integer}インスタンスの辞書である．
...
注：Egisonでは\lstinline{MathValue}，\lstinline{Integer}，\lstinline{Rational}が同一視されるため，\lstinline{isInteger}と\lstinline{isRational}は数式処理システムで数値の性質を判定するために使われる．
