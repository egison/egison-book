...
本章では，（1）簡約規則の宣言，（2）イデアルの宣言による規則集合の機械生成，（3）数学関数の宣言，（4）型注釈による正規形の指定，（5）型の昇格タワーとその拡張，（6）演算の結果型の自動導出，（7）タワーの外に置かれる商の型，の7つを順にみていく．
...
\section{結果型の自動導出}\label{cas-join}

前節の例では，\lstinline{p + q}や\lstinline{a + b}の結果型が，注釈なしで自動的に決まっていた．
$\mathbb{Z}[i]$の値同士の和は$\mathbb{Z}[i]$に閉じ，$\mathbb{Z}[i]$と$\mathbb{Z}[\sqrt{2}]$の和は$\mathbb{Z}[i, \sqrt{2}]$へ昇格する．
本節では，この結果型の自動導出が処理系のなかでどのように実現されているかを解説する．
仕組みは3つの層に分かれている．
（1）同じ型同士の演算は，通常のHindley--Milner型推論（単一化）だけで閉じる．
（2）型が異なるときにだけ，適用位置でjoinが計算され，昇格のためのreshapeが挿入される．
（3）そのjoinの一意性は，前節でみた宣言時の検査が保証している．

出発点として，シンボルの静的型を確認しておく．
第\ref{cas}章でみたように，\lstinline{declare symbol}は\lstinline{declare symbol x : MathValue}のように型を指定でき，省略したときの既定は\lstinline{Integer}である．
つまり注釈のない数式は，タワーの底から静的な世界に入る．
実際，\lstinline{1 + i}の静的型は\lstinline{Integer}であり，CASでない型と混ぜようとすると，そのことがそのまま型エラーに現れる．
\begin{lstlisting}[language=egison,numbers=none]
declare symbol i
...
--   Actual:   Bool (TBool)
\end{lstlisting}
\lstinline{Integer}はタワーのどの型にも埋め込めるので，底から入っておけば，どのCAS型へも注釈ひとつで上がれる．
\ref{cas-tower}節の型注釈が値の「分類」ではなく「組み替え（reshape）」だったのはこのためで，CAS型同士のあいだでは注釈は正規形の選択として常に受理される．
静的な型の精密さは，この注釈と，値から読み取る観察型の2つが与える．

第1の層は，同じ型同士の演算である．
\lstinline{+}や\lstinline{*}は型クラスのメソッドであり，\lstinline{+}の型は\lstinline{class AddSemigroup a}の\lstinline{(+) (x: a) (y: a) : a}，つまり「2つの引数と結果が同じ型」である．
両辺が同じ\lstinline{Poly Integer [i]}なら，単一化で\lstinline{a}が\lstinline{Poly Integer [i]}に決まり，結果型もそのまま$\mathbb{Z}[i]$になる．
joinはまったく関与しない．
\begin{lstlisting}[language=egison,numbers=none]
declare symbol i
declare rule auto term i^2 = -1
...
\end{lstlisting}
このとき残る制約\lstinline{AddSemigroup (Poly Integer [i])}はどう解消されるのだろうか．
ライブラリをみると，CAS型に対する算術インスタンスは\lstinline{instance AddSemigroup MathValue}の1本しかない（\lstinline{Float}などCASの外の型は別として）．
インスタンス解決は，前節の部分型順序（タワー＋宣言辺）の上で「対象の型を包含するもっとも特定的なインスタンス」を選ぶので，\lstinline{Poly Integer [i] <: MathValue}によりこのインスタンスが選ばれる．
静的型は$\mathbb{Z}[i]$のまま精密に保たれ，実装は\lstinline{MathValue}の算術1本に落ちる．
精密な型付けと単一の実装が，部分型順序を介して両立している．
なお，互いに埋め込み合う同格の型（\lstinline{Poly MathValue [..]}と\lstinline{Frac MathValue}など）では特定性だけでは決められないため，対象の表現の頭構成子が一致する側を選ぶ，というタイブレークが入る．

第2の層は，型が異なる場合である．
\lstinline{Poly Integer [i]}と\lstinline{Poly Integer [sqrt2]}のあいだでは\lstinline{a -> a -> a}の単一化が失敗する．
このとき処理系は，失敗した2つのCAS型のjoin（一意な最小上界）を計算し，joinより真に下にある引数へreshapeを挿入したうえで，その適用の推論を一度だけやり直す．
\begin{lstlisting}[language=egison,numbers=none]
...
\end{lstlisting}
重要なのは，単一化器そのものはjoinを知らないことである．
joinは「単一化が失敗したCAS型の組」へのフォールバックとして適用位置に挿入される変換であり，単一化の対称性と主型性は保たれ，CASと無関係な型エラーの報告も影響を受けない．
再試行も失敗した場合には，joinを試みる前の元のエラーがそのまま報告される．

ところで，このjoinが約束しているのは何だろうか．
合併集合の型は，和だけでなく積に対しても正しい．
関係をもたない自由なシンボル同士なら，積が作る\lstinline{s2 s3}は次数2の単項式にすぎず，合併集合のなかに収まる（多項式環として$\mathbb{Z}[s_2] \cdot \mathbb{Z}[s_3] \subseteq \mathbb{Z}[s_2, s_3]$）．
\begin{lstlisting}[language=egison,numbers=none]
...
\end{lstlisting}
事情が変わるのは，\ref{casdetail-ideal}節のイデアルのように，$\sqrt{6}$にあたる新しい原子を右辺にもつ関係を宣言したときである．
生成された規則$s_2 s_3 \to s_6$が積の正規化中に発火し，値は静的型の閉じた原子集合の外へ出る．
\begin{lstlisting}[language=egison,numbers=none]
declare symbol s2, s3, s6
declare ideal [s2^2 - 2, s3^2 - 3, s6 - s2 * s3]
...
\end{lstlisting}
joinが与える静的型は\lstinline{Poly Integer [s2, s3]}のままだが，観察型は\lstinline{s6}を含む．
この不一致は型エラーにならない（\lstinline{Poly Integer [s2, s3]}を明示的に注釈しても同じである）．
閉じた原子集合は，規則を適用する前の構造演算——自由理論——についての契約であり，値レベルの機構である大域規則は型検査器からは見えないからである．
真実は観察型が報告し，規則の発火判定は値ベース（\ref{cas-rule}節のシンボル索引）なので簡約が漏れることもない．
ずれは静的な型表示の粗さに留まり，「静的型は自由理論での予測，観察型が実行後の真実」という分業がここでも働いている．

本物の平方根ではどうなるだろうか．
ここまでの\lstinline{sqrt2}は単なる名前のシンボルだったが，こんどは\ref{casdetail-mathfunc}節の数学関数\lstinline{sqrt}の適用，つまり本物の$\sqrt{2}$を使ってみる．
原子集合には\lstinline{sqrt 2}のような関数適用の原子も書くことができる．
\begin{lstlisting}[language=egison,numbers=none]
...
\end{lstlisting}
静的型はjoinにより合併集合の型\lstinline{Poly Integer [sqrt 3, sqrt 2]}になるが，値は組み込みの融合$\sqrt{2} \cdot \sqrt{3} = \sqrt{6}$によって\lstinline{'sqrt 6}になり，観察型がそれを報告する．
イデアルのときとまったく同じ構図である．
なお，型のなかの関数適用原子は関数名（頭）で照合され，引数までは区別されない．
また，注釈を書かなければ\lstinline{sqrt 2}の静的型は\lstinline{MathValue}（\lstinline{declare mathfunc}の既定）なので，閉じた集合の約束はそもそも発生しない．

第3の層は，前節の宣言時検査そのものである．
第2の層が「一意なjoin」を計算できるのは，どの2つの型にも最小上界がただ1つあるというjoin半束性を，\lstinline{declare cas-subtype}のたびに検査して守っているからである．
ユーザーがタワーをどう拡張しても第2の層の決定性が壊れないのは，この分業による．
また，次節の商の型は名前的（nominal）な型であって順序に参加しないため，joinの対象にもならない．
商の値と生の値の混在が型エラーになるのは，このフォールバックが順序の中の型にしか働かないからである．

まとめると，同じ型は単一化が閉じ（精密さ），異なる型は適用位置のjoinが昇格させ（混在の自動化），宣言時検査がそのjoinの一意性を守る（決定性）．
ここまでにみてきた宣言たちは，この3層の分業の上で互いに干渉せずに済んでいる．
...
最後に，本章の道具立てが合流する例として，位数$4$の有限体$\mathrm{GF}(4) = \mathbb{F}_2[\alpha]/(\alpha^2+\alpha+1)$を作ってみよう．
必要な規律は2つある．係数を$2$で割った余りで数えること（係数の商）と，$\alpha^2 + \alpha + 1 = 0$という関係で$\alpha$の冪を落とすこと（\ref{casdetail-ideal}節のイデアル）である．
ライブラリ関数\lstinline{finiteFieldReduce}は，素数$p$と最小多項式の生成元からこの2つを合成した簡約関数を作る．
グレブナー基底は宣言時に一度だけ計算され，体の除算にはフェルマーの小定理による逆元が使われる（$p$は素数に限る）．
\begin{lstlisting}[language=egison,numbers=none]
...
reprGF4 (a + a) -- 0
\end{lstlisting}
$(\alpha+1)^2 = \alpha$，$\alpha(\alpha+1) = 1$（互いに逆元），$a + a = 0$（標数$2$）と，体の構造が演算のたびに維持される．

底が\lstinline{MathValue}であることには意味がある．
この型は体のスカラーだけでなく，$\mathrm{GF}(4)$係数の多項式も最初から含んでいる．
簡約関数は項の内側の係数にも規律を適用するからである．
\begin{lstlisting}[language=egison,numbers=none]
declare symbol x, y
...
\end{lstlisting}
1つ目では，係数$3$が$1$に，$\alpha^2$が$\alpha + 1$に，多項式の項の内側で簡約されている．
2つ目は標数$2$の体でだけ成り立つ恒等式$(u+v)^2 = u^2 + v^2$である．
係数の商（本節）とイデアルによる規則生成（\ref{casdetail-ideal}節）という2つの直交した機構が，簡約関数の合成だけで有限体に組み上がる．
