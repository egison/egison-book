...
The spherical coordinate system consists of three parameters: $r$, $\theta$, and $\phi$.
$r$ represents the distance from the origin.
$\theta$ represents the angle from the $z$-axis, i.e., the latitude.
$\phi$ represents the rotation from the $x$-axis in the $xy$-plane, i.e., the longitude.
...
First, the basis vectors of the tangent space at each point on the sphere determined by $\theta$ and $\phi$ are computed.
...
def g_i_j : Tensor MathValue :=
  [| [| `(c^2 * r - 2 * G * M) / (c^2 * r), 0, 0, 0 |]
   ...
   |]
\end{lstlisting}
The complete program is available at \texttt{sample/math/geometry/riemann-curvature-tensor-of-Schwarzschild-metric.egi} in the Egison source code.
...
--    [| (-1 / 2) * ('cos `$\theta$`) * `$\pi$`^-1, 0 |] |]
\end{lstlisting}
Here, \lstinline[mathescape]{$\Omega$} is the curvature form antisymmetrized over its indices.
The complete program is available at \texttt{sample/math/geometry/euler-form-of-T2.egi} in the Egison source code.
...
The interior product is defined in the library (\texttt{lib/math/geometry/differential-form.egi}) as follows.
\lstinline{DiffForm a} is a built-in type representing differential forms.
\begin{lstlisting}[language=egison,numbers=none]
def `$\iota$` {Field a} (X: DiffForm a) (Y: DiffForm a) : DiffForm a :=
  withSymbols [i] dfOrder Y * (X...~i . dfNormalize Y..._i)
\end{lstlisting}
The Lie derivative can be defined as follows (\lstinline{N} is the dimension of the manifold).
\begin{lstlisting}[language=egison,numbers=none]
